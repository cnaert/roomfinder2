...
def on_request(body):
    ...
    if cmd == "book":
    	...
    	page = requests.post("127.0.0.1:8080"+'/book',data = json.dumps(request_data),headers=headers)
        txt=page.text
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r","--rabbitmq", help="IP or hostname for rabbitmq server, e.g. 'rabbit.domain.com'.",required=False)
    parser.add_argument("-p","--port", help="tcp port for rabitmq server, e.g. '2765'.",required=False)
    ...
    parser.add_argument("-i", "--photo", help="Address of photo directory server", required=False)
    parser.add_argument("-u","--user", help="URL for user of directory server.", required=False)
    parser.add_argument("-k","--password", help="URL for password of directory server.", required=False)
    parser.add_argument("-s","--sso", help="URL for SSO.", required=False)
